...
var accordion = function accordion(triggersSelector, itemsSelector) {
  //подправить стили : offset-questions
  ...
  }); // blocks.forEach(block => { // использование CSS
...
var calc = function calc(state) {
  ...
  var cleareInputs = function cleareInputs() {
  ...
  uploadBtn.forEach(function (item) {
...
/* harmony import */ var _services_request__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../services/request */ "./src/js/services/request.js");
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      ...
      if (input.closest('.main')) {
        ...
        Object(_services_request__WEBPACK_IMPORTED_MODULE_6__["postData"])('assets/server.php', formData).then(function (result) {
          console.log(result);
        }).catch(function () {
          console.log('Error');
        }).finally(function () {
...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (e) {
    var target = e.target;
    var classSelect = target.classList[0];
    var allElems = wrapper.querySelectorAll(".".concat(classSelect));
    typeFilter(allElems);

    if (target && target.tagName == "LI") {
      //делегирование и назначение класса активности
...
/* harmony import */ var core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_5__);
/* harmony import */ var _services_request__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../services/request */ "./src/js/services/request.js");
/* harmony import */ var _calc__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./calc */ "./src/js/modules/calc.js");
...
var forms = function forms(costPicture) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      if (item.closest('.calc')) {
        for (var key in costPicture) {
          formData.append(key, costPicture[key]);
        }
      }

      Object(_services_request__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        console.log(res);
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      ...
      // ручной полифил для старых браузеров
  ...
  function createMask(event) {
    var matrix = '+375 (__) ___ __ __',
        //можно вруную, можно через джейсон
    i = 0,
...
var modals = function modals() {
  var btnPressed; //отслеживание любых нажатий
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      //проверка долистал ли пользователь до конца страницы
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    ...
    img.src = img.src.slice(0, -4) + '-1.png'; //отрезаем 4 символа .png

    block.querySelectorAll('p:not(.sizes-hit)').forEach(function (p) {
      //полезное дополнение, позволяет выбрать вск, кроме :not
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); // cards.forEach(card => { //простой вариант, когда есть все в верстке
